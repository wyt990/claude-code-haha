...
项目已内置多 Agent 协作能力（不是靠技能“开功能”）。是否自动并行取决于当前模式、任务类型与调度策略；你也可以手动明确要求分工并行。下面给出 3 种最稳妥的使用姿势：

### 场景 1：并行调研（最安全，优先推荐）

**适用**：需求还不清晰、要快速摸清多个模块。

**做法**：先让多个 Agent 分头做只读调研（不改代码），主 Agent 汇总后再决定实现方案。

**示例提示词：**

```text
请并行启动 3 个 agent 做只读调研，不要改代码：
1) 梳理认证与权限相关文件和调用链
2) 梳理会话存储与消息流转路径
3) 梳理 CLI 命令注册与执行入口
最后汇总：影响面、风险点、建议实施顺序。
...
**适用**：需要实际改动代码，又希望降低冲突和回滚成本。

**做法**：只让 1 个 Agent 负责写代码；其它 Agent 并行做验证（类型检查、构建、关键流程回归）。

**示例提示词：**

```text
按“单写多验”执行：
- Agent A：负责实现改动（仅改指定目录）
- Agent B：只做 typecheck，并定位新引入的报错
- Agent C：只做构建与关键命令回归
最后给出：改动摘要、验证结果、剩余风险。
```
...
**做法**：先拆分为可独立合入的工作单元，再并行执行；每个单元都要求独立验证，避免“全部做完才发现不可合并”。

**示例提示词：**

```text
将本次改造拆成可独立合入的 4 个工作单元并并行推进：
- 先给出拆分方案（每个单元的目录范围、目标、验证命令）
- 再分别执行实现与验证
- 输出最终汇总：每个单元的完成状态、验证证据、建议合并顺序
```

### 额外建议

- 想要可控：优先显式写出“谁负责实现、谁负责验证”。
- 先调研后实现：并行调研通常比直接并行改代码更稳。
- 并行写代码时尽量按目录隔离，减少冲突。
- 技能用于“流程模板和方法论增强”，不是底层能力开关。
