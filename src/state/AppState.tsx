...
export function useAppState<T>(selector: (state: AppState) => T): T {
  ...
  const getSnapshot = useCallback(() => {
    ...
    if (MACRO.BUILD_IS_ANT && state === selected) {
      throw new Error(
        `Your selector in \`useAppState(${String(selectorRef.current)})\` returned the original state, which is not allowed. You must instead return a property for optimised rendering.`,
      );
    }
    return selected;
  }, [store]);
