...
function PromptInput({
  ...
  const hasTungstenSession = useAppState(s => MACRO.BUILD_IS_ANT && s.tungstenActiveSession !== undefined);
  const tmuxFooterVisible = MACRO.BUILD_IS_ANT && hasTungstenSession;
  ...
  const hasBgTaskPill = useMemo(() => Object.values(tasks).some(t => isBackgroundTask(t) && !(MACRO.BUILD_IS_ANT && isPanelAgentTask(t))), [tasks]);
  ...
  const tasksFooterVisible = (runningTaskCount > 0 || MACRO.BUILD_IS_ANT && coordinatorTaskCount > 0) && !shouldHideTasksFooter(tasks, showSpinnerTree);
  ...
  useKeybindings({
    'footer:up': () => {
      ...
      if (tasksSelected && MACRO.BUILD_IS_ANT && coordinatorTaskCount > 0 && coordinatorTaskIndex > minCoordinatorIndex) {
    ...
    'footer:down': () => {
      ...
      if (tasksSelected && MACRO.BUILD_IS_ANT && coordinatorTaskCount > 0) {
