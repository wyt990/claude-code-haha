...
export function ConfirmStepWrapper({
  ...
  const saveAgent = useCallback(async (openInEditor: boolean): Promise<void> => {
    ...
    try {
      ...
      const message = openInEditor ? `已创建 agent：${chalk.bold(wizardData.finalAgent.agentType)} 并在编辑器中打开。` + `如果您进行了编辑，请重启以加载最新版本。` : `已创建 agent：${chalk.bold(wizardData.finalAgent.agentType)}`;
      onComplete(message);
    } catch (err) {
