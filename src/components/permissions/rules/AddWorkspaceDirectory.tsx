...
function PermissionDescription() {
  ...
  if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
    t0 = <Text dimColor={true}>开启自动接受编辑后，Claude Code 将能读取此目录中的文件并进行编辑。</Text>;
    $[0] = t0;
  } else {
    t0 = $[0];
  }
...
export function AddWorkspaceDirectory(t0) {
  ...
  if ($[27] !== onCancel || $[28] !== t10 || $[29] !== t11) {
    t12 = <Dialog title="将目录添加到工作区" onCancel={onCancel} color="permission" isCancelActive={false} inputGuide={t10}>{t11}</Dialog>;
    $[27] = onCancel;
    $[28] = t10;
    $[29] = t11;
    $[30] = t12;
...
function _temp2(exitState) {
  return exitState.pending ? <Text>再按一次 {exitState.keyName} 退出</Text> : <Byline><KeyboardShortcutHint shortcut="Tab" action="complete" /><KeyboardShortcutHint shortcut="Enter" action="add" /><ConfigurableShortcutHint action="confirm:no" context="Settings" fallback="Esc" description="取消" /></Byline>;
}
